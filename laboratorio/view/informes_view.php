 ...
 * informes_view.php
 *
 * Item "Informes" del sidebar (grupo "Calidad y entrega"). Contiene dos
 * piezas reales:
 *
 *   1) El buscador/exportador de resultados por lote y tipo de reporte
 *      (suelos/aguas/foliares/cana), que ya existia en este modulo bajo el
 *      nombre de archivo view/dashboard.php: varias paginas del modulo
 *      (view/labc_index.php, view/menu_solicitud.php) ya enlazaban a ese
 *      archivo con la etiqueta "Informes"/"Historial", asi que esta pagina
 *      es la continuacion real de esa funcionalidad, no una reconstruccion
 *      desde cero. view/dashboard.php ahora es un dashboard de KPIs nuevo
 *      (ver ese archivo), por lo que este buscador se movio aqui.
 *   2) Un listado de "Documentos generados" (tipo_documento = 'informe')
 *      desde la tabla lab_documento (database/002_lab_flujo_generico.sql).
 *      Ningun flujo del modulo escribe en esa tabla todavia, asi que el
 *      listado parte vacio de forma honesta (sin datos de ejemplo).
 */
...
function informes_build_url(string $tipoReporte, ?int $idLote = null): string
{
...
function informes_report_label(string $tipoReporte): string
{
    switch ($tipoReporte) {
        case 'suelos':
            return 'Suelos';
...
foreach ($reportTypes as $key => &$meta) {
    $meta['href'] = informes_build_url($key, $idLoteSelected);
    $meta['total_configurados'] = count($analysisMap[$key] ?? []);
    $meta['active'] = ($key === $tipoReporte);
}
unset($meta);

function informes_table_columns(PDO $pdo, string $table): array
{
    static $cache = [];
...
function informes_get_lote_sample_keys(PDO $pdo, int $idLote): array
{
    static $cache = [];
...
function informes_count_analysis_rows(PDO $pdo, $tables, int $idLote): int
{
    $tables = is_array($tables) ? $tables : [$tables];
    $sampleKeys = null;

    foreach ($tables as $table) {
        $columns = informes_table_columns($pdo, $table);
        if (!$columns) {
            continue;
        }
        ...
        if (in_array('no_lab', $columns, true) || in_array('numero_laboratorio', $columns, true) || in_array('numero_muestra', $columns, true)) {
            $sampleKeys ??= informes_get_lote_sample_keys($pdo, $idLote);
            if ($sampleKeys) {
    ...
    if ($lote) {
        ...
        foreach ($analysisMap[$tipoReporte] as $key => $analisis) {
            try {
                ...
                $total = informes_count_analysis_rows($pdo, $tablas, $idLoteSelected);
...
$documentosInforme = lab_documentos_listar($pdo, 'informe');
$tablaDocumentosDisponible = lab_documentos_tabla_existe($pdo, 'lab_documento');

function informes_fecha_legible($fecha): string
{
...
?>
<?php
lab_shell_head('Informes', 'Consulta y exporta resultados por lote, y revisa los informes generados', [
    '../styles/base.css',
    '../css/dashboard.css?v=1',
    'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css',
]);
lab_shell_open('informes_view.php');
?>
            <div class="content-shell">
                <div class="topbar-actions">
                    <a href="../index.php" class="topbar-link">Inicio</a>
                    <a href="<?= htmlspecialchars(informes_build_url($tipoReporte, $idLoteSelected)) ?>" class="topbar-link active">Informes</a>
                ...
                <section class="page-intro">
                    ...
                    <div class="hero-copy-block">
                        <p class="hero-kicker">Informes de laboratorio</p>
                        ...
                        <div class="no-data">
                            <div>
                                ...
                                <p class="no-data-copy">
                                    ...
                                    en la categoría <strong><?= htmlspecialchars(informes_report_label($tipoReporte)) ?></strong>.
                ...
                <section class="cengi-card" style="margin-top:28px;">
                    <h3>Documentos generados (informes)</h3>
                    <div class="cengi-card-sub">
                        Informes versionados registrados en <code>lab_documento</code>. Ningun flujo del
                        modulo escribe todavia en esta tabla, asi que este listado parte vacio de forma
                        honesta hasta que exista un generador de informes en PDF persistido.
                    </div>

                    <?php if (!$tablaDocumentosDisponible): ?>
                        <div class="cengi-empty">
                            La tabla <code>lab_documento</code> todavia no existe en esta base de datos.
                            Aplique <code>laboratorio/database/002_lab_flujo_generico.sql</code>.
                        </div>
                    <?php elseif (empty($documentosInforme)): ?>
                        <div class="cengi-empty">Todavia no se ha generado ningun informe versionado.</div>
                    <?php else: ?>
                        <div class="cengi-table-wrap">
                            <table>
                                <thead>
                                    <tr><th>Lote</th><th>Titulo</th><th>Version</th><th>Vigente</th><th>Generado por</th><th>Generado en</th></tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($documentosInforme as $doc): ?>
                                        <tr>
                                            <td>Lote <?= htmlspecialchars($doc['codigo_lote'] ?? '-', ENT_QUOTES, 'UTF-8') ?></td>
                                            <td><?= htmlspecialchars($doc['titulo'] ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                            <td>v<?= (int) $doc['version'] ?></td>
                                            <td><?= !empty($doc['vigente']) ? 'Si' : 'No' ?></td>
                                            <td><?= htmlspecialchars($doc['generado_por'] ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                            <td><?= htmlspecialchars(informes_fecha_legible($doc['generado_en'] ?? null), ENT_QUOTES, 'UTF-8') ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
